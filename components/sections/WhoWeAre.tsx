...
function formatParagraph(text: string) {
    const keywords = [
        "World Federation of Shotokan Karate (WFSK)",
        "India, UAE, Qatar, Saudi Arabia, Bahrain, U.K and Gambia",
        "Karate-Do, self-defense, oriental weapons, and fitness",
        "WFSK is committed to discipline, excellence, and the true spirit of Shotokan Karate.",
    ...
    keywords.forEach((keyword) => {
        const newFormatted: React.ReactNode[] = [];
            ...
            if (typeof node === "string") {
                ...
                if (parts.length > 1) {
                    parts.forEach((part, i) => {
                        if (i > 0) {
                            newFormatted.push(
                                <strong key={i} className="font-semibold text-[#7C7C7C]">
                                    {keyword}
                                </strong>
                            );
                        }
